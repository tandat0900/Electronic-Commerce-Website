...
export class OrderComponent implements OnInit {
  ...
  placeOrder() {
    ...
    if (!isPaymentSuccessfull) {
      ...
      this.message = 'Đã xảy ra sự cố! Thanh toán đã không xảy ra!';
    ...
    let count = timer(0, 3000).subscribe((res) => {
      ++step;
      if (step === 1) {
        this.message = 'xử lý thanh toán';
        this.classname = 'text-success';
      }
      if (step === 2) {
        this.message = 'Thanh toán thành công, Đơn hàng đang được đặt.';
        this.storeOrder();
      }
      if (step === 3) {
        this.message = 'Đơn đặt hàng của bạn đã được đặt';
        this.displaySpinner = false;
      }
      if (step === 4) {
